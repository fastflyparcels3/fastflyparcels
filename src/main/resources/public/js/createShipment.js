  ...
  statusesXhr.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      response.forEach(function (status) {
        ...
        document.getElementById("status").innerHTML += bindStatus(
          status,
          selected
        );
      });
...
document.body.addEventListener("click", function (e) {
  ...
  if (target.classList.contains("order-stage-icon")) {
    deleteStage(target.parentElement.previousElementSibling.innerText);
  ...
  } else if (target.id == "add-stage-btn") {
    openModal("add-stage-modal");
  } else if (target.id == "close-stage-modal") {
...
function addShipmentStage() {
  ...
  shipmentStagesTemp.push(shipmentStageEtx.value);
  let status = "check";
  shipmentStagesTemp.forEach(function (item, index) {
    if (index == shipmentStagesTemp.length - 1) {
      status = "local_shipping";
    }
    document.getElementById("shipment-stage-root").innerHTML +=
...
function deleteStage(stage) {
  shipmentStagesTemp.forEach(function (item, index) {
    if (item == stage) {
      shipmentStagesTemp.splice(index, 1);
    }
  });
  document.getElementById("shipment-stage-root").innerHTML = "";
  let status = "check";
...
function addShipment() {
  ...
  let addShipmentXhr = new XMLHttpRequest();
  addShipmentXhr.open("POST", "/shipment", true);
  addShipmentXhr.setRequestHeader("Content-type", "application/json");
  addShipmentXhr.send(JSON.stringify(shipment));

  addShipmentXhr.onreadystatechange = function () {
